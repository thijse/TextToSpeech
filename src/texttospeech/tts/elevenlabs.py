...
class ElevenLabsTTS(TTSInterface):
    ...
    def text_to_speech(self, text, voice_name, output_path, output_format="mp3_44100_128", is_ssml=False):
        ...
        Args:
            ...
            is_ssml (bool, optional): Whether the text contains SSML markup. 
                                     Note: ElevenLabs doesn't support SSML, so this will be ignored
                                     and plain text will be extracted.
        ...
        try:
            ...
            if is_ssml:
                # Simple SSML tag removal for ElevenLabs
                import re
                # Remove SSML tags but keep the text content
                clean_text = re.sub(r'<[^>]+>', '', text)
                text = clean_text.strip()
                print(f"Note: ElevenLabs doesn't support SSML. Using plain text: {text[:50]}...")
